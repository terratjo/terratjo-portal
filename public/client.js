...
window.openIPM = function(id) {
  ...
  const createdDateRaw = b.created_at ? b.created_at.split(' ')[0] : _todayJkt;
...
function printInvoice() {
  ...
  let paymentHtml = '';
  if (b.status === 'confirmed' && b.paymentMethod) {
    paymentHtml = `<div class="pm"><strong>Method Used:</strong> ${b.paymentMethod}</div>`;
  } else {
  ...
  const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Terratjo '+type+'</title>'
    ...
    + '<div><div class="dt">'+type+'</div><div class="dm" style="font-weight:700; color:#1a1a2e; margin-bottom: 2px;">'+b.id+'</div><div class="dm">Date: '+ (b.created_at ? fmtDate(b.created_at.split(' ')[0]) : todayStr) +'</div></div></div>'
